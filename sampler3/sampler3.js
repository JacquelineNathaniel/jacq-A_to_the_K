...
cnv.mousePressed(userStartAudio);
 word = myTextList[i];
  ...
  points = font.textToPoints(
    word, 0, 0, map(micVolume,0,1,80,200), {
      sampleFactor: 0.75,
      simplifyThreshold: 0
    });
    
    bounds = font.textBounds(word, 0, 0, map(micVolume,0,1,80,200));
  cursor(CROSS);
  fill(255, 127);
  noStroke();
}
...
textSize(map(micVolume,0,1,80,200));
textFont(font);
text(myText, 0, 0);
pop();
}
   ...
   pop();
  let centerDist = dist(map(micVolume,0,1,80,200), width / 2, height / 2);
